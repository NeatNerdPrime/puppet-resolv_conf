...
describe 'resolv_conf' do
  on_supported_os.each do |os, facts|
    ...
    context "on #{os} with three nameservers" do
      let(:params) do
        { nameservers: %w[8.8.8.8 8.8.4.4 9.9.9.9] }
      end
    ...
    context "on #{os} with four nameservers" do
      let(:params) do
        { nameservers: %w[8.8.8.8 8.8.4.4 9.9.9.9 9.9.9.10] }
      end
    ...
    context "on #{os} with truthy prepend_local_nameserver" do
      let(:params) do
        { nameservers: %w[8.8.8.8], prepend_local_nameserver: true }
      end
    ...
    context "on #{os} with falsy prepend_local_nameserver" do
      let(:params) do
        { nameservers: %w[8.8.8.8], prepend_local_nameserver: false }
      end
    ...
    context "on #{os} with domainname and searchlist" do
      let(:params) do
        { domainname: 'example.com', searchlist: %w[example.com example.net] }
      end
    ...
    context "on #{os} with valid parameter options" do
      let(:params) do
        { options: %w[ndots:1 timeout:30 attempts:5] }
      end
